...
| 3 | `mb_weights_round3.npz` | *pending* | *pending* (see `docs/training.md` §5) | *pending* | *pending* | *pending* | round-3 reward scheme, reveal mask, strengthened direction channels, RL fine-tune — see `docs/training.md` |

Round 2 is the currently installed policy; the 30-seed number is on the lucky side of the
100-seed one, so quote **0.48 ± 0.05** (binomial SE) as its win rate. Round 1 is kept because it is
the checkpoint the round-1 evaluation in `docs/training.md` §3.5 was run on (0 wins; the first
...
```bash
scripts/install_weights.sh round2          # copies models/mb_weights_round2.npz -> data/compiled/mb_weights.npz
scripts/install_weights.sh round1          # the round-1 checkpoint instead
...
python -m flysweeper.mb_eval models/mb_weights_round2.npz --games 30 --agree-turns 600   # agreement + confusion + games
```
...
| `params` | JSON: the full `MBParams` (pools, `extra_orn`, `kc_kc_gain`, `kc_bias`, `pn_bias`, `pn_kc_gain`, `reveal_margin`, rewards, learning rate, clip range, ...) |
